...
def main(args):
    ...
    for person_id in sorted(os.listdir(args.input_dir)):
        ...
        output_images2 = list()
        # pdb.set_trace()
        # 如果同时传入4张图像，那么num_samples必须是1，否则会爆显存
        # output_images = ip_model.generate(pil_image=images, num_samples=1, num_inference_steps=30, seed=42, prompt="a photo of person")
        # pdb.set_trace()
        for image in images:
            # pdb.set_trace()
            output_image1 = ip_model.generate(pil_image=image, num_samples=2, num_inference_steps=30, seed=42, prompt="a photo of person")
            output_image1 += ip_model.generate(pil_image=image, num_samples=2, num_inference_steps=30, seed=41, prompt="a photo of person")
            output_images1 += output_image1
