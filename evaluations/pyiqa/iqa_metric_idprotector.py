...
def main():
    ...
    ori_persons_list = os.listdir(args.data_dirs)
    persons_list = list()
    for person_id in ori_persons_list:
        person_path = os.path.join(args.data_dirs, person_id)
    ...
    with torch.no_grad():
        for k, person_id in enumerate(persons_list):
            ...
            for idx, id_img in enumerate(img_list):
            ...
            ave_lpips_score = ave_lpips_score / len(img_list)
            ave_ssim_score = ave_ssim_score / len(img_list)
            ave_psnr_score = ave_psnr_score / len(img_list)
            ave_liqe_score = ave_liqe_score / len(img_list)
            ave_brisque_score = ave_brisque_score / len(img_list)
