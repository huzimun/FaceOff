...
class PhotoMakerStableDiffusionXLPipeline(StableDiffusionXLPipeline):
    ...
    def encode_prompt_with_trigger_word(
        ...
        if prompt_embeds is None:
            ...
            for prompt, tokenizer, text_encoder in zip(prompts, tokenizers, text_encoders):
                ...
                pooled_prompt_embeds = prompt_embeds[0] # torch.Size([1, 77, 768])
                prompt_embeds = prompt_embeds.hidden_states[-2] # torch.Size([1, 77, 768])
                prompt_embeds_list.append(prompt_embeds)

            prompt_embeds = torch.concat(prompt_embeds_list, dim=-1)

        prompt_embeds = prompt_embeds.to(dtype=self.text_encoder_2.dtype, device=device) # torch.Size([1, 77, 1280])
    ...
    def __call__(
        ...
        (
            ...
            pooled_prompt_embeds=pooled_prompt_embeds_text_only, # torch.Size([4, 1280])
        ...
        if gaussian_filter == 1 or hflip == 1:
            if gaussian_filter == 1 and hflip == 1:
                trans = [T.GaussianBlur(kernel_size=7,), T.RandomHorizontalFlip(p=0.5)]
                print("Conduct GaussianBlur with kernel size 7 and RandomHorizontalFlip with probability 0.5")
            elif gaussian_filter == 1:
                trans = [T.GaussianBlur(kernel_size=7,)]
                print("Conduct GaussianBlur with kernel size 7")
            elif hflip == 1:
                trans = [T.RandomHorizontalFlip(p=0.5)]
                print("RandomHorizontalFlip with probability 0.5")
            trans = T.Compose(trans)
            id_pixel_values = trans(id_pixel_values)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i <= start_merge_step:
                    ...
                    add_text_embeds = torch.cat([negative_pooled_prompt_embeds, pooled_prompt_embeds], dim=0) # torch.Size([8, 1280])
                # predict the noise residual
                added_cond_kwargs = {"text_embeds": add_text_embeds, "time_ids": add_time_ids} # add_text_embeds: torch.Size([8, 1280]), add_time_ids: torch.Size([8, 6])
                noise_pred = self.unet(
                    latent_model_input, # torch.Size([8, 4, 128, 128])
                    t,# tensor(981, device='cuda:0')
                    encoder_hidden_states=current_prompt_embeds, # torch.Size([8, 77, 2048])
