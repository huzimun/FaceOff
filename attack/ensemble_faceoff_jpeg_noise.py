...
# 初始化可微分JPEG压缩器
jpeg = DifferentiableJPEG(quality=70, chroma_subsampling="420")
...
def add_gaussian_noise(image_tensor, mean=0.0, std=0.1):
    ...
    参数:
        ...
        mean: 高斯噪声的均值，默认0
        std: 高斯噪声的标准差（控制噪声强度），默认0.1
    返回:
        加噪后的图像张量，与输入形状相同，梯度可传递
    """
    # 生成与图像同形状的高斯噪声（使用torch.randn保证可微分）
    noise = torch.randn_like(image_tensor) * std + mean
    
    # 叠加噪声（核心操作，可微分）
    noisy_image = image_tensor + noise
    
    # # 可选：裁剪像素值到合理范围（如[0,1]），clamp操作同样可微分
    noisy_image = torch.clamp(noisy_image, image_tensor.min(), image_tensor.max())
    
    return noisy_image

# 多模型攻击，对多个图像编码器进行攻击
def pgd_ensemble_attack(eot_trans_types,
                model_dict, # 模型池, key为模型类型，value为模型
                perturbed_data,
                origin_data,
                alpha,
                eps,
    ...
    with torch.no_grad():
        ...
        if "jpeg" in eot_trans_types.split(','):
            tran_target_data = jpeg.compress(target_data)
            tran_origin_data = jpeg.compress(origin_data)
            eot_trans_target_data_list.append(tran_target_data)
            eot_trans_origin_data_list.append(tran_origin_data)
        if "noise" in eot_trans_types.split(','):
            tran_target_data = add_gaussian_noise(target_data)
            tran_origin_data = add_gaussian_noise(origin_data)
            eot_trans_target_data_list.append(tran_target_data)
            eot_trans_origin_data_list.append(tran_origin_data)
        ...
        for k in model_dict.keys():
            ...
            for i in range(0, len(eot_trans_target_data_list)):
    ...
    for epoch in range(0, attack_num):
        ...
        if "jpeg" in eot_trans_types.split(','):
            tran_perturbed_data = jpeg.compress(perturbed_data)
            eot_trans_perturbed_data_list.append(tran_perturbed_data)
        if "noise" in eot_trans_types.split(','):
            tran_perturbed_data = add_gaussian_noise(perturbed_data)
            eot_trans_perturbed_data_list.append(tran_perturbed_data)
        Loss_x_ = []
        Loss_d_ = []
        for k in model_dict.keys():
            ...
            loss_x_list = list()
            loss_d_list = list()
            for i in range(0, len(eot_trans_perturbed_data_list)): # 每个trans下的loss累加作为该模型对应的loss
...
def main(args):
    ...
    for person_id in sorted(os.listdir(args.data_dir)):
        ...
        adv_data, Loss_dict = pgd_ensemble_attack(args.eot_trans_types,
                model_dict, # 模型池, key为模型类型，value为模型
                perturbed_data,
                origin_data,
                args.alpha*255,
