...
def main(args):
    ...
    for person_id in sorted(os.listdir(args.input_dir)):
        ...
        output_images2 = list()
        for image in images:
            output_image1 = ip_model.generate(pil_image=image, num_samples=4, num_inference_steps=30, seed=42, prompt="a photo of person")
            output_images1 += output_image1
            torch.cuda.empty_cache()
        for image in images:
            output_image2 = ip_model.generate(pil_image=image, num_samples=4, num_inference_steps=30, seed=42, prompt="a dslr portrait of person")
